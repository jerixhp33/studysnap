...
export async function POST(req: NextRequest) {
  try {
    ...
    if (input.data.document_id) {
      let { data: doc } = await supabase.from('documents').select('extracted_text, name').eq('id', input.data.document_id).eq('user_id', user.id).single()
      if (!doc) {
        try {
          const admin = createAdminClient()
          const { data: adminDoc } = await admin.from('documents').select('extracted_text, name').eq('id', input.data.document_id).eq('user_id', user.id).single()
          doc = adminDoc
      ...
      const docIds = (await supabase.from('documents').select('id').eq('subject_id', input.data.subject_id).eq('user_id', user.id)).data?.map(d => d.id) ?? []
      const { data: chunks } = await supabase.from('document_chunks').select('content').eq('user_id', user.id).in('document_id', docIds).limit(20)
    ...
    try { await db.from('ai_generations').insert({ user_id: user.id, feature: 'flashcards', model: 'llama-3.3-70b-versatile' }) } catch {}
